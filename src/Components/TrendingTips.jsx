    ...
    return (
        <div className="mt-20 mx-4 max-w-6xl lg:mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {tips.map(tip => (
                    <div key={tip._id} className=" rounded-xl p-4 shadow-2xl bg-white text-center place-items-center ">
                        <img src={tip.image} alt={tip.title} className="w-30 h-30 object-cover rounded-full " />
                        <h3 className="text-xl font-semibold mt-3 text-green-800">{tip.title}</h3>
                        <p className="text-green-800 opacity-60 mt-2 text-center">{tip.description.slice(0, 60)}...</p>
